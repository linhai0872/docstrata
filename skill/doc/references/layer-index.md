...
给 coding agent 的四层文档**导航索引**——轻量指针，指向各层，不复制内容。让 agent 启动时知道"项目的知识在哪、需要时去哪读"。
...
INDEX.md 没有 completeness contract、不 GRILL。它从**已存在的层文档**提炼，是机械派生：

1. 扫描 `docs/` 下存在哪些层文档（wiki/requirements/knowledge/dev）。
...
实证（arXiv:2602.11988）：臃肿的 agent context 文件会降低成功率、增加成本。INDEX.md 必须克制：
...
操作约定（构建/测试/红线）：见 AGENTS.md / CLAUDE.md（若存在）。
