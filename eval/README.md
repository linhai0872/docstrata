...
给 **skill 作者**用的开发期工具：改了 `methodology.md` 或某层契约后，验证这次迭代是真改进还是退步。它不进用户的安装包，对用户零负担。

docstrata 的迭代过去全靠"我觉得变好了"。这套评测把"觉得"换成可重复的分数——改动前后跑同一组项目，对比每层得分。

## 设计取舍

- **reference-free**：judge 拿「生成的文档 + 项目上下文」按 rubric 打分，不维护"标准答案"。项目本身就是 ground truth，省掉最大的人工黑洞。
- **三段分档，能力按需付费**：结构硬门(零依赖) → 单 judge(快) → 多 judge ensemble(严谨)。
...
硬门查固定 section 骨架齐不齐、`last-verified` 在不在，不过门直接红灯。
judge 按 5 维度各 0-3 打分（契约覆盖度 / 来源忠实度 / 认知状态准确 / 受众适配 / 层专属），归一化 ≥ 0.7 为 pass。
...
`judges.yaml` 默认给一组去相关 ensemble（DeepSeek / OpenAI / 阿里，经 ZenMux 聚合）。换成自己的端点照文件里的注释范例改 `base_url` + `model` + `api_key_env`。启用 1 个 = 快速档，启用 ≥2 个 = ensemble。
...
**grill 式复盘**——评测的收尾。把 `/grill-with-docs` 的范式（一次一个、附推荐答案、人确认比从零做省力）套到"改进"上：agent 负责**发现+起草**，人只**决策**。跑完评测后，把下面这段交给 agent：
...
判"真改进 vs 噪声"：v1 用简易法——同一文档跑 2-3 次看分数波动（judge 有 temperature=0 但仍有微抖动），波动盖过的差异不算数。积累十几轮数据后再考虑算正式噪声基线（σ₀）。
