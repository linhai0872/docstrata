...
一个 Claude Code Skill（skill 名 `docstrata`，用户命令 `/docstrata`），按知识性质分层为**任意项目类型**生成/维护文档。各层是一个子命令，共享同一套「探索→评估缺口→grill→生成→留痕」执行循环。交付形态是 Agent Skill（开放标准，跨 agent 平台），CLI 留待后续。

## 命令清单（完整）

| 命令 | 层 / 类型 | 产出文件 |
|---|---|---|
| `prd` | 产品主张（前瞻 intent，非 CoALA 记忆） | `docs/prd.md` |
...
| `compact` | 横切收缩操作（非生成层） | 收缩已有层文档 |
| `all` | 全部 | 五层 + index |

- `prd` 是最上游的产品意图层（前瞻主张，可重写），区别于 requirements 的已发生事实（只增）。不属 CoALA 长期记忆三类。
- `all` 真实存在，按依赖顺序一次生成：`prd → requirements → knowledge → wiki → dev → index`。无实质价值的层（如无业务材料的 knowledge）按「优雅降级」跳过并说明。
- `index` 不走五步循环（无 completeness contract，是纯派生），但遵循增量更新与 STAMP。
- `compact` 是横切收缩操作（手动触发）：先压（丢 code/git 可复原项 + 去重 + 按主题归并）、再拆、归档不删，治长期记忆膨胀。不是生成层。
...
```
skill/docstrata/SKILL.md              # 子命令路由 + 五步循环骨架
skill/docstrata/references/           # methodology / source-criticism / doc-conventions /
